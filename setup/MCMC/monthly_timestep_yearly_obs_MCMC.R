## file: monthly_timestep_yearly_obs_MCMC.R
...
## create daily C matrix
C <- cbind(Cf_e, Cw_e, Cr_e, Clit_e, Csom_e)

## create monthly C matrix
C <- C[cumsum(rep(c(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31), 6)), ]

## create observations
C_obs <- matrix(NA, ncol = 5, nrow = 72)
C_obs[c(seq(from = 1, to = 72, by = 12), 72), 1] <- Cf.obs[cumsum(rep(c(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31), 6))[c(seq(from = 1, to = 72, by = 12), 72)]]
C_obs[c(seq(from = 1, to = 72, by = 12), 72), 2] <- Cw.obs[cumsum(rep(c(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31), 6))[c(seq(from = 1, to = 72, by = 12), 72)]]
C_obs[c(seq(from = 1, to = 72, by = 12), 72), 3] <- Cr.obs[cumsum(rep(c(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31), 6))[c(seq(from = 1, to = 72, by = 12), 72)]]
C_obs[c(seq(from = 1, to = 72, by = 12), 72), 4] <- Clit.obs[cumsum(rep(c(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31), 6))[c(seq(from = 1, to = 72, by = 12), 72)]]
C_obs[c(seq(from = 1, to = 72, by = 12), 72), 5] <- Csom.obs[cumsum(rep(c(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31), 6))[c(seq(from = 1, to = 72, by = 12), 72)]]
...
if (!('stock_inits' %in% ls())){
  if (!('init_type' %in% ls())){
    ## use flux data to initialize latent states
    ...
    ## recursive for loop to estimate latent states using flux data
    for (i in 1:72){
      if (i %in% missing_data_ind){
        ...
        testCf[i] <- testCf[i-1] + sum(smooth(Af_obs)[(csnd[i-1]+1):csnd[i]]) - sum(smooth(Lf_obs)[(csnd[i-1]+1):csnd[i]])
        testCr[i] <- testCr[i-1] + sum(smooth(Ar_obs)[(csnd[i-1]+1):csnd[i]]) - sum(smooth(Lr_obs)[(csnd[i-1]+1):csnd[i]])
        testCw[i] <- testCw[i-1] + sum(smooth(Aw_obs)[(csnd[i-1]+1):csnd[i]]) - sum(smooth(Lw_obs)[(csnd[i-1]+1):csnd[i]])
        testClit[i] <- testClit[i-1] + sum(smooth(Lf_obs)[(csnd[i-1]+1):csnd[i]]) + sum(smooth(Lr_obs)[(csnd[i-1]+1):csnd[i]]) - 
          sum(smooth(Rh1_obs)[(csnd[i-1]+1):csnd[i]]) - sum(smooth(D_e_obs)[(csnd[i-1]+1):csnd[i]])
        testCsom[i] <- testCsom[i-1] + sum(smooth(D_e_obs)[(csnd[i-1]+1):csnd[i]]) + sum(smooth(Lw_obs)[(csnd[i-1]+1):csnd[i]]) -
          sum(smooth(Rh2_obs)[(csnd[i-1]+1):csnd[i]])
      }
    }
    ## store flux data initializations
    ...
    cat('Initializing latent states using flux data\n')
...
## initial process model predictions
